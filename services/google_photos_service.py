...
class GooglePhotosService:
    ...
    def list_photos(self, page_size: int = 100) -> List[Dict]:
        ...
        try:
            while True:
                logger.info(f"Fetching photos (page size: {page_size}, token: {next_page_token})")
    ...
    def download_photo(
        ...
        try:
            ...
            # Add =d parameter to get full-quality download
            ...
            # Save metadata as JSON sidecar if requested
            metadata_file = None
            if preserve_metadata:
                metadata_file = output_path.with_stem(output_path.stem + '_metadata')
                metadata = {
                    'photo_id': photo_id,
                    'filename': filename,
    ...
    def download_all_photos(
        ...
        Args:
            ...
            batch_size: Number of photos to show in progress indicators
            preserve_metadata: Whether to save metadata for each photo
            order_by: Sort order - "creationTime" (oldest first) or "relevance" (recent first)
        ...
        try:
            ...
            if order_by == "creationTime":
                logger.info("Sorting photos by creation time (oldest first)...")
                photos.sort(key=lambda p: p.get('mediaMetadata', {}).get('creationTime', ''), reverse=False)
                logger.info("Sorted by creation time (oldest first)")
            else:
                logger.info("Using relevance order (recent first)")
            
            # Limit if requested
            if max_photos:
                photos = photos[:max_photos]
                logger.info(f"Limited to {max_photos} photos for download")
            ...
            logger.info(f"Starting download of {stats['total']} photos...")
            
            for idx, photo in enumerate(photos, 1):
                if idx % batch_size == 0 or idx == 1:
                    logger.info(f"Progress: {idx}/{stats['total']}")
                
                # Skip non-photo media (videos, etc.)
                mime_type = photo.get('mimeType', '')
                if not mime_type.startswith('image/'):
                    logger.debug(f"Skipping non-image media: {mime_type}")
                    stats['skipped'] += 1
                    continue
            ...
            logger.info(f"Skipped (non-images): {stats['skipped']}")
